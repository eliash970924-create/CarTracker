...
/**
 * CarTally's neutrals - page, cards, borders, secondary text - shared by the
 * garage, Settings and every car.
 *
 * Light: ivory page, white cards, warm hairlines. Dark: the night page and
 * its cards. Material's own neutrals lean towards its default purple and read
 * as lavender-grey, which is what the car screens used to wear while the
 * garage had moved on.
 */
private fun carTallyNeutrals(base: ColorScheme, dark: Boolean): ColorScheme =
    if (dark) {
        val faint = lerp(CarTallyNight, CarTallyIvory, 0.15f)
        base.copy(
        ...
        base.copy(
            ...
            surfaceVariant = CarTallyHairline,
            surfaceContainerLowest = Color.White,
            surfaceContainerLow = Color.White,
            surfaceContainer = Color.White,
            surfaceContainerHigh = Color.White,
            surfaceContainerHighest = Color.White,
            outline = CarTallyOutline,
            outlineVariant = CarTallyHairline
        )
    }

/**
 * The garage's colours: CarTally's own, from the logo, over its neutrals.
 *
 * Light: petrol bar, ivory page, white cards - the mockups' layout.
 *
...
fun carTallyColorScheme(base: ColorScheme, dark: Boolean): ColorScheme {
    val neutrals = carTallyNeutrals(base, dark)
    return if (dark) {
        neutrals.copy(
...
/**
 * A car's colours: its own accent, over CarTally's neutrals.
 *
 ...
 *
 * The accent is exactly as chosen in light mode. In dark mode it is lifted
...
fun carColorScheme(base: ColorScheme, accent: Color, dark: Boolean): ColorScheme {
    val neutrals = carTallyNeutrals(base, dark)
    val primary = if (dark) liftUntilReadable(accent, neutrals.surface) else accent
    val lightPrimary = primary.luminance() > 0.5f
    return neutrals.copy(
        primary = primary,
        onPrimary = if (lightPrimary) Color.Black else Color.White,
        primaryContainer = lerp(neutrals.surface, accent, if (dark) 0.3f else 0.2f),
        onPrimaryContainer = when {
            dark -> neutrals.onSurface
            lightPrimary -> Color(0xFF1A1A1A)
            else -> accent
        },
        secondaryContainer = lerp(neutrals.surface, accent, if (dark) 0.18f else 0.1f)
    )
}
